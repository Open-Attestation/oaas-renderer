import * as React from 'react'
import renderer from 'react-test-renderer'

import { sample } from '../free-sale-medical-devices.sample'
import { HealthSciencesAuthorityFreeSaleMedicalDevicesTemplate } from '../free-sale-medical-devices.template'

describe('HealthSciencesAuthorityFreeSaleMedicalDevicesTemplate', () => {
    it('should match snapshot', () => {
        const tree = renderer
            .create(
                <HealthSciencesAuthorityFreeSaleMedicalDevicesTemplate
                    document={sample}
                    handleObfuscation={() => void 0}
                />
            )
            .toJSON()

        expect(tree).toMatchInlineSnapshot(`
            .c1 {
              background: white;
              position: relative;
              width: 21cm;
              min-height: 29.7cm;
              padding: 0.1px;
              margin: 0 auto;
              box-sizing: border-box;
              border: 1px solid #ccc;
              background-size: cover;
              background-image: url('medicaldevice-a4-bg.png');
              z-index: initial;
            }

            .c24 {
              background: white;
              position: relative;
              width: 21cm;
              min-height: 29.7cm;
              padding: 0;
              margin: 0 auto;
              box-sizing: border-box;
              border: 1px solid #ccc;
              background-size: cover;
              background-image: url('undefined');
              z-index: initial;
            }

            .c0 {
              -webkit-transform: scale(1);
              -ms-transform: scale(1);
              transform: scale(1);
              -webkit-transform-origin: top left;
              -ms-transform-origin: top left;
              transform-origin: top left;
              height: 4506px;
            }

            .c2 {
              margin-left: 96px;
              margin-right: 96px;
              margin-top: 258px;
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: column;
              -ms-flex-direction: column;
              flex-direction: column;
              -webkit-align-items: center;
              -webkit-box-align: center;
              -ms-flex-align: center;
              align-items: center;
              padding: 0px;
              gap: 36px;
            }

            .c3 {
              -webkit-flex: none;
              -ms-flex: none;
              flex: none;
              -webkit-order: 0;
              -ms-flex-order: 0;
              order: 0;
              -webkit-align-self: stretch;
              -ms-flex-item-align: stretch;
              align-self: stretch;
              -webkit-box-flex: 0;
              -webkit-flex-grow: 0;
              -ms-flex-positive: 0;
              flex-grow: 0;
              z-index: 0;
            }

            .c4 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: row;
              -ms-flex-direction: row;
              flex-direction: row;
              -webkit-box-pack: justify;
              -webkit-justify-content: space-between;
              -ms-flex-pack: justify;
              justify-content: space-between;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
              padding: 0px;
              gap: 33px;
            }

            .c5 {
              -webkit-flex: none;
              -ms-flex: none;
              flex: none;
              -webkit-order: 0;
              -ms-flex-order: 0;
              order: 0;
              -webkit-align-self: stretch;
              -ms-flex-item-align: stretch;
              align-self: stretch;
              -webkit-box-flex: 0;
              -webkit-flex-grow: 0;
              -ms-flex-positive: 0;
              flex-grow: 0;
              font-family: 'Times New Roman';
              font-style: normal;
              font-weight: 400;
              font-size: 16px;
              line-height: 120%;
              color: #000000;
            }

            .c6 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: column;
              -ms-flex-direction: column;
              flex-direction: column;
              -webkit-align-items: center;
              -webkit-box-align: center;
              -ms-flex-align: center;
              align-items: center;
              padding: 0px;
              gap: 18px;
            }

            .c7 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: column;
              -ms-flex-direction: column;
              flex-direction: column;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
              padding: 0px;
              gap: 20px;
            }

            .c8 {
              -webkit-flex: none;
              -ms-flex: none;
              flex: none;
              -webkit-order: 0;
              -ms-flex-order: 0;
              order: 0;
              -webkit-align-self: stretch;
              -ms-flex-item-align: stretch;
              align-self: stretch;
              -webkit-box-flex: 0;
              -webkit-flex-grow: 0;
              -ms-flex-positive: 0;
              flex-grow: 0;
              font-family: 'Times New Roman';
              font-style: normal;
              font-weight: 400;
              font-size: 16px;
              line-height: 120%;
              color: #000000;
            }

            .c9 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: column;
              -ms-flex-direction: column;
              flex-direction: column;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
              gap: 12px;
              font-family: 'Times New Roman';
              font-style: italic;
              font-weight: 400;
              font-size: 12px;
              line-height: 120%;
              color: #000000;
            }

            .c10 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: column;
              -ms-flex-direction: column;
              flex-direction: column;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
            }

            .c11 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: row;
              -ms-flex-direction: row;
              flex-direction: row;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
              gap: 20px;
            }

            .c12 {
              width: 190px;
            }

            .c13 {
              width: 284px;
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: row;
              -ms-flex-direction: row;
              flex-direction: row;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
              gap: 20px;
            }

            .c14 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: row;
              -ms-flex-direction: row;
              flex-direction: row;
              -webkit-box-pack: center;
              -webkit-justify-content: center;
              -ms-flex-pack: center;
              justify-content: center;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
              padding: 0px;
              gap: 40px;
            }

            .c15 {
              -webkit-flex: none;
              -ms-flex: none;
              flex: none;
              -webkit-order: 2;
              -ms-flex-order: 2;
              order: 2;
              -webkit-align-self: stretch;
              -ms-flex-item-align: stretch;
              align-self: stretch;
              -webkit-box-flex: 0;
              -webkit-flex-grow: 0;
              -ms-flex-positive: 0;
              flex-grow: 0;
            }

            .c16 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: column;
              -ms-flex-direction: column;
              flex-direction: column;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
              padding: 0px;
              gap: 31px;
              -webkit-align-self: stretch;
              -ms-flex-item-align: stretch;
              align-self: stretch;
            }

            .c17 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: column;
              -ms-flex-direction: column;
              flex-direction: column;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
              padding: 0px;
              gap: 16px;
            }

            .c18 {
              width: 185px;
              height: 92px;
              text-align: center;
              -webkit-align-items: flex-end;
              -webkit-box-align: flex-end;
              -ms-flex-align: flex-end;
              align-items: flex-end;
              -webkit-box-pack: center;
              -webkit-justify-content: center;
              -ms-flex-pack: center;
              justify-content: center;
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
            }

            .c19 {
              display: block;
              max-width: 185px;
              max-height: 92px;
            }

            .c20 {
              font-family: 'Times New Roman';
              font-style: normal;
              font-weight: 400;
              font-size: 14.3449px;
              line-height: 115%;
              -webkit-letter-spacing: -0.005em;
              -moz-letter-spacing: -0.005em;
              -ms-letter-spacing: -0.005em;
              letter-spacing: -0.005em;
              color: #000000;
            }

            .c21 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: column;
              -ms-flex-direction: column;
              flex-direction: column;
              -webkit-align-items: center;
              -webkit-box-align: center;
              -ms-flex-align: center;
              align-items: center;
              padding: 0px;
              gap: 11px;
            }

            .c22 {
              font-family: 'Times New Roman';
              font-style: italic;
              font-weight: 400;
              font-size: 12px;
              line-height: 120%;
              color: #6b7280;
            }

            .c23 {
              -webkit-align-self: stretch;
              -ms-flex-item-align: stretch;
              align-self: stretch;
              font-family: 'Times New Roman';
              font-style: normal;
              font-weight: 400;
              font-size: 10px;
              line-height: 120%;
              text-align: center;
              color: #000000;
            }

            .c26 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: column;
              -ms-flex-direction: column;
              flex-direction: column;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
              padding: 100px 96px 50px;
              gap: 24px;
            }

            .c27 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: row;
              -ms-flex-direction: row;
              flex-direction: row;
              width: 100%;
            }

            .c28 {
              -webkit-box-flex: 1;
              -webkit-flex-grow: 1;
              -ms-flex-positive: 1;
              flex-grow: 1;
              height: 81.58px;
              width: 143px;
            }

            .c25 {
              position: absolute;
              top: 122px;
              right: 158px;
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: column;
              -ms-flex-direction: column;
              flex-direction: column;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
              font-family: 'Times New Roman';
              font-style: normal;
              font-weight: 400;
              font-size: 16px;
              line-height: 120%;
              color: #000000;
            }

            .c29 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: column;
              -ms-flex-direction: column;
              flex-direction: column;
              width: 100%;
              gap: 18px;
            }

            .c30 {
              font-family: 'Times New Roman';
              font-style: normal;
              font-weight: 700;
              font-size: 16px;
              line-height: 120%;
              text-align: center;
              color: #000000;
            }

            .c31 {
              font-family: 'Times New Roman';
              font-style: normal;
              font-weight: 400;
              font-size: 16px;
              line-height: 120%;
              color: #000000;
            }

            .c31 table {
              border-collapse: collapse;
            }

            .c31 th,
            .c31 td {
              border: 1px solid black;
              padding: 10px 8px;
            }

            .c32 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: row;
              -ms-flex-direction: row;
              flex-direction: row;
              -webkit-box-pack: end;
              -webkit-justify-content: flex-end;
              -ms-flex-pack: end;
              justify-content: flex-end;
              width: 100%;
            }

            .c33 {
              margin-top: 8.42px;
              margin-right: 34.73px;
              height: 94.27px;
              width: 95.29px;
            }

            .c34 {
              position: absolute;
              left: 96px;
              right: 96px;
              bottom: 50px;
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: row;
              -ms-flex-direction: row;
              flex-direction: row;
              -webkit-box-pack: justify;
              -webkit-justify-content: space-between;
              -ms-flex-pack: justify;
              justify-content: space-between;
              font-family: 'Times New Roman';
              font-style: normal;
              font-weight: 400;
              font-size: 12px;
              line-height: 120%;
              color: #000000;
            }

            .c35 {
              margin-bottom: 8px;
              margin-left: 16px;
              font-family: 'Times New Roman';
              font-style: normal;
              font-weight: 400;
              font-size: 16px;
              line-height: 120%;
              color: #000000;
            }

            .c36 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: column;
              -ms-flex-direction: column;
              flex-direction: column;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
              padding: 96px 96px 50px;
              gap: 40px;
              font-family: 'Times New Roman';
              font-style: normal;
              font-weight: 400;
              font-size: 16px;
              line-height: 120%;
              color: #000000;
            }

            .c37 {
              width: 100%;
              font-weight: 700;
              text-align: center;
            }

            .c38 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: column;
              -ms-flex-direction: column;
              flex-direction: column;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
              gap: 32px;
            }

            .c39 {
              width: 100% display:flex;
              -webkit-flex-direction: column;
              -ms-flex-direction: column;
              flex-direction: column;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
            }

            .c40 {
              display: -webkit-box;
              display: -webkit-flex;
              display: -ms-flexbox;
              display: flex;
              -webkit-flex-direction: row;
              -ms-flex-direction: row;
              flex-direction: row;
              -webkit-align-items: flex-start;
              -webkit-box-align: flex-start;
              -ms-flex-align: flex-start;
              align-items: flex-start;
              padding: 0px 0px 0px 20px;
              gap: 8px;
            }

            @media print {
              .c1 {
                border: none;
                -webkit-break-inside: avoid;
                break-inside: avoid;
              }
            }

            @media print {
              .c24 {
                border: none;
                -webkit-break-inside: avoid;
                break-inside: avoid;
              }
            }

            @media print {
              .c0 {
                -webkit-transform: scale(1);
                -ms-transform: scale(1);
                transform: scale(1);
                height: auto;
              }
            }

            <div
              className="c0"
            >
              <div
                className="c1"
              >
                <div
                  className="c2"
                >
                  <div
                    className="c3"
                  >
                    <div
                      className="c4"
                    >
                      <div
                        className="c5"
                      >
                        HSA 600:36/18
                        <br />
                        Date of Issue: 
                        31 Dec 2023
                      </div>
                      <div
                        className="c5"
                      >
                        Certificate No.
                        <br />
                        <b>
                          FSC/258/2023
                        </b>
                      </div>
                    </div>
                  </div>
                  <div
                    className="c3"
                  >
                    <div
                      className="c6"
                    >
                      <div
                        className="c7"
                      >
                        <div
                          className="c8"
                          style={
                            Object {
                              "textAlign": "center",
                            }
                          }
                        >
                          <b>
                            CERTIFICATE OF FREE SALE FOR MEDICAL DEVICE
                          </b>
                        </div>
                        <div
                          className="c8"
                        >
                          It is hereby certified that the following medical device products may be supplied in Singapore subject to the device being listed on the Singapore Medical Device Register.
                        </div>
                      </div>
                      <div
                        className="c9"
                      >
                        <div
                          className="c10"
                        >
                          <div
                            className="c11"
                          >
                            <div
                              className="c12"
                            >
                              Device Proprietary/Brand Name
                            </div>
                            <div
                              className="c13"
                            >
                              <div>
                                :
                              </div>
                              <div>
                                Please refer to attached Schedule.
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          className="c10"
                        >
                          <div
                            className="c11"
                          >
                            <div
                              className="c12"
                            >
                              Intended Use
                            </div>
                            <div
                              className="c13"
                            >
                              <div>
                                :
                              </div>
                              <div>
                                Please refer to attached Schedule.
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          className="c10"
                        >
                          <div
                            className="c11"
                          >
                            <div
                              className="c12"
                            >
                              Manufacturing Site
                            </div>
                            <div
                              className="c13"
                            >
                              <div>
                                :
                              </div>
                              <div>
                                GMC INC
                                <br />
                                NO.686, SU CHU RD, CHUZHOU, ANHUI, CHINA
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          className="c10"
                        >
                          <div
                            className="c11"
                          >
                            <div
                              className="c12"
                            >
                              Product Owner
                            </div>
                            <div
                              className="c13"
                            >
                              <div>
                                :
                              </div>
                              <div>
                                ROSSMAX SWISS GMBH
                                <br />
                                WIDNAUERSTRASSE 1, CH-9435, HEERBRUGG, SWITZERLAND
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          className="c10"
                        >
                          <div
                            className="c11"
                          >
                            <div
                              className="c12"
                            >
                              Registrant
                            </div>
                            <div
                              className="c13"
                            >
                              <div>
                                :
                              </div>
                              <div>
                                BION ADVANCE PTE. LTD
                                <br />
                                35 PIONEER ROAD NORTH, #02-03 SINGAPORE 628475
                              </div>
                            </div>
                          </div>
                        </div>
                        <div>
                          Please refer to the attached definition of the terms used in this certificate
                        </div>
                      </div>
                      <div
                        className="c14"
                      >
                        <div
                          className="c15"
                        >
                          2
                        </div>
                        <div
                          className="c15"
                          style={
                            Object {
                              "flex": "1",
                            }
                          }
                        >
                          The information provided in this certificate and attached Schedule is based on the information submitted to the Authority.
                        </div>
                      </div>
                      <div
                        className="c14"
                      >
                        <div
                          className="c15"
                        >
                          3
                        </div>
                        <div
                          className="c15"
                          style={
                            Object {
                              "flex": "1",
                            }
                          }
                        >
                          The information provided in this certificate and attached Schedule is accurate as on the date of issuance.
                        </div>
                      </div>
                      <div
                        className="c16"
                      >
                        <div
                          className="c17"
                        >
                          <div
                            className="c18"
                          >
                            <img
                              alt="Signature of signee"
                              className="c19"
                              src="data:image/png;base64,"
                            />
                          </div>
                          <div
                            className="c20"
                          >
                            DR LAKSHMIDEVI BALAKRISHNAN
                            <br />
                            REGULATORY CONSULTANT
                            <br />
                            for GROUP DIRECTOR
                            <br />
                            HEALTH PRODUCTS REGULATION GROUP
                            <br />
                            HEALTH SCIENCES AUTHORITY
                          </div>
                        </div>
                        <div
                          className="c21"
                        >
                          <div
                            className="c22"
                          >
                            This certificate shall not be construed as an endorsement of the medical device product by the Health Sciences Authority
                          </div>
                          <div
                            className="c23"
                          >
                            Medical Devices Cluster, Health Products Regulation Group, Health Sciences Authority
                            <br />
                            11 Biopolis Way, #11-01 Helios, Singapore 138667. Tel: (65) 6866 1111, Fax: (65) 6478 9028
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <img
                  alt="HSA stamp"
                  src="hsa-stamp.svg"
                  style={
                    Object {
                      "height": "94.27px",
                      "left": "572px",
                      "position": "absolute",
                      "top": "818px",
                      "width": "95.29px",
                    }
                  }
                />
              </div>
              <div
                className="c24"
              >
                <div
                  className="c25"
                >
                  Certificate No.
                  <br />
                  <b>
                    FSC/258/2023
                  </b>
                </div>
                <div
                  className="c26"
                >
                  <div
                    className="c27"
                  >
                    <img
                      alt="HSA Logo"
                      className="c28"
                      src="logo-hsa.svg"
                    />
                  </div>
                  <div
                    className="c29"
                  >
                    <div
                      className="c30"
                    >
                      THE SCHEDULE
                    </div>
                    <div
                      className="c31"
                    >
                      <table
                        style={
                          Object {
                            "width": "100%",
                          }
                        }
                      >
                        <tr>
                          <th>
                            No.
                          </th>
                          <th
                            style={
                              Object {
                                "width": "240px",
                              }
                            }
                          >
                            Device Proprietary/Brand Name
                          </th>
                          <th>
                            Intended Use
                          </th>
                        </tr>
                        <tr>
                          <td
                            style={
                              Object {
                                "textAlign": "center",
                              }
                            }
                          >
                            1
                            .
                          </td>
                          <td>
                            Rossmax Blood Pressure Monitors(Upper Arm)
                          </td>
                          <td>
                            Intended for measuring systolic and diastolic blood pressure using the oscillometric method for adults at home.
                          </td>
                        </tr>
                        <tr>
                          <td
                            style={
                              Object {
                                "textAlign": "center",
                              }
                            }
                          >
                            2
                            .
                          </td>
                          <td>
                            Rossmax Handheld Pulse Oximeter
                          </td>
                          <td>
                            Intended for measuring SpO2, pulse rate & artery condition readings at home.
                          </td>
                        </tr>
                      </table>
                    </div>
                    <div
                      className="c30"
                    >
                      END OF PRODUCT LIST
                    </div>
                  </div>
                  <div
                    className="c32"
                  >
                    <img
                      alt="HSA Stamp"
                      className="c33"
                      src="hsa-stamp.svg"
                    />
                  </div>
                </div>
                <div
                  className="c34"
                >
                  <div>
                    HSA 600:36/18 dated 
                    31 Dec 2023
                  </div>
                  <div>
                    Page 1 of 2
                  </div>
                </div>
              </div>
              <div
                className="c24"
              >
                <div
                  className="c25"
                >
                  Certificate No.
                  <br />
                  <b>
                    FSC/258/2023
                  </b>
                </div>
                <div
                  className="c26"
                >
                  <div
                    className="c27"
                  >
                    <img
                      alt="HSA Logo"
                      className="c28"
                      src="logo-hsa.svg"
                    />
                  </div>
                  <div
                    className="c29"
                  >
                    <div
                      className="c30"
                    >
                      APPENDIX I
                    </div>
                    <div
                      className="c31"
                    >
                      <div
                        className="c35"
                      >
                        <b>
                          1
                          . 
                          Rossmax Blood Pressure Monitors(Upper Arm)
                        </b>
                      </div>
                      <table
                        style={
                          Object {
                            "marginBottom": "24px",
                            "width": "100%",
                          }
                        }
                      >
                        <tr>
                          <th>
                            S/No.
                          </th>
                          <th
                            style={
                              Object {
                                "textAlign": "left",
                              }
                            }
                          >
                            Model/ Description
                          </th>
                          <th
                            style={
                              Object {
                                "textAlign": "left",
                              }
                            }
                          >
                            Product Number
                          </th>
                        </tr>
                        <tr>
                          <td
                            style={
                              Object {
                                "textAlign": "center",
                              }
                            }
                          >
                            1
                            .
                          </td>
                          <td>
                            Rossmax CF155f
                          </td>
                          <td>
                            CF155f
                          </td>
                        </tr>
                        <tr>
                          <td
                            style={
                              Object {
                                "textAlign": "center",
                              }
                            }
                          >
                            2
                            .
                          </td>
                          <td>
                            Rossmax CF100g
                          </td>
                          <td>
                            CF100g
                          </td>
                        </tr>
                      </table>
                      <div
                        className="c35"
                      >
                        <b>
                          2
                          . 
                          Rossmax Handheld Pulse Oximeter
                        </b>
                      </div>
                      <table
                        style={
                          Object {
                            "marginBottom": "24px",
                            "width": "100%",
                          }
                        }
                      >
                        <tr>
                          <th>
                            S/No.
                          </th>
                          <th
                            style={
                              Object {
                                "textAlign": "left",
                              }
                            }
                          >
                            Model/ Description
                          </th>
                          <th
                            style={
                              Object {
                                "textAlign": "left",
                              }
                            }
                          >
                            Product Number
                          </th>
                        </tr>
                        <tr>
                          <td
                            style={
                              Object {
                                "textAlign": "center",
                              }
                            }
                          >
                            1
                            .
                          </td>
                          <td>
                            Rossmax SA300
                          </td>
                          <td>
                            SA300
                          </td>
                        </tr>
                      </table>
                    </div>
                    <div
                      className="c30"
                    >
                      END OF PRODUCT LIST
                    </div>
                  </div>
                  <div
                    className="c32"
                  >
                    <img
                      alt="HSA Stamp"
                      className="c33"
                      src="hsa-stamp.svg"
                    />
                  </div>
                </div>
                <div
                  className="c34"
                >
                  <div>
                    HSA 600:36/18 dated 
                    31 Dec 2023
                  </div>
                  <div>
                    Page 2 of 2
                  </div>
                </div>
              </div>
              <div
                className="c24"
              >
                <div
                  className="c36"
                >
                  <div
                    className="c37"
                  >
                    Definition of Terms
                  </div>
                  <div
                    className="c38"
                  >
                    <div
                      className="c39"
                    >
                      <b>
                        Manufacturing Site:
                      </b>
                      <br />
                      The location where the manufacture of a health product is carried out. Manufacture, in relation to a health product, means to make, fabricate, produce or process the health product and includes —
                      <div
                        className="c40"
                      >
                        <div>
                          (a)
                        </div>
                        <div>
                          any process carried out in the course of so making, fabricating, producing or processing the health product; and
                        </div>
                      </div>
                      <div
                        className="c40"
                      >
                        <div>
                          (b)
                        </div>
                        <div>
                          the packaging and labelling of the health product before it is supplied
                        </div>
                      </div>
                    </div>
                    <div
                      className="c39"
                    >
                      <b>
                        Product Owner:
                      </b>
                      <br />
                      in relation to a health product, means a person who –
                      <div
                        className="c40"
                      >
                        <div>
                          (a)
                        </div>
                        <div>
                          supplies the health product under his own name, or under any trade-mark, design, trade name or other name or mark owned or controlled by him; and
                        </div>
                      </div>
                      <div
                        className="c40"
                      >
                        <div>
                          (b)
                        </div>
                        <div>
                          is responsible for designing, manufacturing, assembling, processing, labelling, packaging, refurbishing or modifying the health product, or for assigning to it a purpose, whether those tasks are performed by him or on his behalf
                        </div>
                      </div>
                    </div>
                    <div
                      className="c39"
                    >
                      <b>
                        Registrant:
                      </b>
                      <br />
                      in relation to a registered health product, means the person who applied for and obtained the registration of the health product under the Health Products Act
                    </div>
                    <div
                      className="c39"
                    >
                      <b>
                        Supply:
                      </b>
                      <br />
                      in relation to a health product, means to transfer possession of the health product by any means whether or not for reward, and includes the following:
                      <div
                        className="c40"
                      >
                        <div>
                          (a)
                        </div>
                        <div>
                          to sell the health product, whether by retail, wholesale or auction;
                        </div>
                      </div>
                      <div
                        className="c40"
                      >
                        <div>
                          (b)
                        </div>
                        <div>
                          to expose or display the health product as an invitation to treat;
                        </div>
                      </div>
                      <div
                        className="c40"
                      >
                        <div>
                          (c)
                        </div>
                        <div>
                          to transfer possession of the health product by exchange, gift, lease, loan, hire or hire-purchase;
                        </div>
                      </div>
                      <div
                        className="c40"
                      >
                        <div>
                          (d)
                        </div>
                        <div>
                          to supply the health product in connection with —
                        </div>
                      </div>
                      <div
                        className="c40"
                        style={
                          Object {
                            "padding": "0px 0px 0px 44px",
                          }
                        }
                      >
                        <div>
                          (i)
                        </div>
                        <div>
                          a contract for the provision of any goods or the performance of any service; or
                        </div>
                      </div>
                      <div
                        className="c40"
                        style={
                          Object {
                            "padding": "0px 0px 0px 44px",
                          }
                        }
                      >
                        <div>
                          (ii)
                        </div>
                        <div>
                          any advertising, sponsorship or promotional activity;
                        </div>
                      </div>
                      <div
                        className="c40"
                      >
                        <div>
                          (e)
                        </div>
                        <div>
                          to supply the health product by way of administration to or application in any person in the course of any diagnosis, treatment or test;
                        </div>
                      </div>
                      <div
                        className="c40"
                      >
                        <div>
                          (f)
                        </div>
                        <div>
                          to offer, agree or attempt to supply the health product in any of the ways described in paragraphs (a) to (e) or to cause or permit the health product to be so supplied; and
                        </div>
                      </div>
                      <div
                        className="c40"
                      >
                        <div>
                          (g)
                        </div>
                        <div>
                          to keep or possess the health product for the purpose of supplying it in any of the ways described in paragraphs (a) to (f)
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <img
                  alt="HSA stamp"
                  src="hsa-stamp.svg"
                  style={
                    Object {
                      "height": "94.27px",
                      "left": "572px",
                      "position": "absolute",
                      "top": "912px",
                      "width": "95.29px",
                    }
                  }
                />
              </div>
            </div>
        `)
    })
})
